    ...
    def run(
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                raw_image_msg = self.bridge.cv2_to_imgmsg(im0, encoding="bgr8")
                self.publisher_raw_image.publish(raw_image_msg)
                ...
                model_image_msg = self.bridge.cv2_to_imgmsg(im0, encoding="bgr8")
            
            # Publish the annotated image with bounding boxes
                self.publisher_ml.publish(model_image_msg)
                if view_img:
